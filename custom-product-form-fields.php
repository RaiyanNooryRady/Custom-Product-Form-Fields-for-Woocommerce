...
<?php
/*
Plugin Name: Custom Product Form Fields PRO (DOM Fixed)
...
<div class="cpff-form" style="border:1px solid #ccc;padding:10px;margin-bottom:20px;">
    <label>Form Name:</label>
    <input type="text" name="cpff_forms[<?php echo $formIndex; ?>][name]" value="<?php echo esc_attr($form['name']); ?>" style="width:200px;"><br>
    <strong>Assign to Categories:</strong><br>
    <?php foreach ($categories as $cat): ?>
        <label>
            <input type="checkbox" name="cpff_forms[<?php echo $formIndex; ?>][categories][]" value="<?php echo esc_attr($cat->term_id); ?>"
            <?php if (!empty($form['categories']) && in_array($cat->term_id, $form['categories'])) echo 'checked'; ?>>
            <?php echo esc_html($cat->name); ?>
        </label><br>
    <?php endforeach; ?>
    <h4>Form Fields:</h4>
    <div class="cpff-fields" data-form="<?php echo $formIndex; ?>">
        <?php if (!empty($form['fields'])): foreach ($form['fields'] as $fieldIndex => $field): ?>
            <div class="cpff-field" style="border:1px solid #ddd;padding:10px;margin-bottom:10px;">
                <button type="button" class="remove-field button" onclick="this.closest('.cpff-field').remove()">❌</button><br>
                ...
                <input type="text" name="cpff_forms[<?php echo $formIndex; ?>][fields][<?php echo $fieldIndex; ?>][label]" value="<?php echo esc_attr($field['label']); ?>" placeholder="Label">
                <textarea name="cpff_forms[<?php echo $formIndex; ?>][fields][<?php echo $fieldIndex; ?>][options]" placeholder="Options (label:price)"><?php echo esc_textarea($field['options'] ?? ''); ?></textarea>
            </div>
...
add_action('woocommerce_before_add_to_cart_button', function () {
    ...
    foreach ($forms as $formIndex => $form) {
        if (!isset($form['categories'], $form['fields'])) continue;
